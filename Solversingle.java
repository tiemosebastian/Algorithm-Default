...
public class Solversingle {
	final Boardsingle game;
	final Boardsingle twin;
	...
	//////////////////////////////////////////////////////////////////////////////////////////
    public Solversingle(Boardsingle initial){           // find a solution to the initial board (using the A* algorithm)
    	game=initial;
    	twin=game.twin();
    ...
    public Iterable<Boardsingle> solution() {     // sequence of boards in a shortest solution; null if unsolvable
    	if(!isSolvable()) return null;
    	return new Solve();
    }
    public static void main(String[] args){ // solve a slider puzzle (given below)
    	int testarray[] = new int[]{1,2,3,4,5,6,7,8,0};
    	Solversingle test=new Solversingle(new Boardsingle(testarray));
    	Stopwatch timer = new Stopwatch();
    	for(int i=0;i<1000;i++)
    	{
    		StdRandom.shuffle(testarray);
    		test = new Solversingle(new Boardsingle(testarray));
    	}
    	StdOut.print(timer.elapsedTime()+"\n");
    	if(test.isSolvable()){
    		for(Boardsingle i : test.solution()){
       ...
       /* In in = new In(args[0]);
        ...
        Boardsingle initial = new Boardsingle(blocks);

        // solve the puzzle
        Solversingle solver = new Solversingle(initial);
        ...
        if (!solver.isSolvable())
            ...
            for (Boardsingle board : solver.solution())
	...
	private class Solve implements Iterable<Boardsingle>{
    	...
    	class SolveIterator implements Iterator<Boardsingle>{
    ...
    class BoardNode{
    	public Boardsingle current;
    	public BoardNode previous;
    	public BoardNode[] neighbors;
    	public final int moves;
    	public final int manpri;
    	
    	public BoardNode(Boardsingle current, BoardNode previous, int manpri){
    	...
    	public BoardNode(Boardsingle current, BoardNode previous){
    		this(current, previous, -1000);
    	}
    	public BoardNode(Boardsingle current){
    		this(current, null);
    	}
    	public void findNeighbors(){
    		int i=0;
    		neighbors = new BoardNode[4];
    		for(Boardsingle p : current.neighbors()){
